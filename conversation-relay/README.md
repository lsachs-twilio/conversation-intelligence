...
This server also exposes two endpoints for the Conversation Insights Plugin to use. The get-recommendations endpoint surfaces a recommended audience to engage based on the conversation trends. The create-audience endpoint creates the recommended audience within Segment. 
...
- Segment Account
- OpenAI Account
- Snowflake Account (if running the recommendation endpoint)
...
pnpm dev
```
NOTE: if you are using a snowflake account id, the app will print out your recommendations endpoint for use the insights plugin config
